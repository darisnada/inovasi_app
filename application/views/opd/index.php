...
<script>
    $('.add-data').on('click', function() {
        $('.modal-title').text('Tambah OPD Baru')
        $('form').attr('action', '<?= base_url('opd/store') ?>')
        $('#myModal').modal('show');
    })
    $('.edit-data').on('click', function() {
        $('.modal-title').text('Edit OPD')
        let id = $(this).data('id');
        $('form').attr('action', `<?= base_url('opd/update/') ?>${id}`)
        $.ajax({
            url: "<?= base_url('opd/show/') ?>" + id,
            method: "GET",
    ...
    $('.delete-data').on('click', function() {
        ...
        if (res == true) {
            $.ajax({
                url: "<?= base_url('opd/destroy/') ?>" + $(this).data('id'),
                method: "GET",
                success: function(result) {

...
<script>
    $('.add-data').on('click', function() {
        $('.modal-title').text('Tambah ASN Baru')
        $('input[name="name"]').val('')
        ...
        $('form').attr('action', '<?= base_url('asn/store') ?>')
        $('#myModal').modal('show');
    })
    $('.edit-data').on('click', function() {
        $('.modal-title').text('Edit ASN')
        let id = $(this).data('id');
        $('form').attr('action', `<?= base_url('asn/update/') ?>${id}`)
        $.ajax({
            url: "<?= base_url('asn/show/') ?>" + id,
            method: "GET",
            success: function(result) {
                let obj = JSON.parse(result)
                console.log(obj.agency_id)
    ...
    $('.delete-data').on('click', function() {
        ...
        if (res == true) {
            $.ajax({
                url: "<?= base_url('asn/destroy/') ?>" + $(this).data('id'),
                method: "GET",
                success: function(result) {
